var gatling = (() => {
  ...
  var require_checks = __commonJS({
    "node_modules/@gatling.io/core/target/checks/index.js"(exports) {
      ...
      exports.xpath = xpath;
      var css4 = (selector, nodeAttribute) => {
      ...
      var jmesPath2 = (path) => (0, jsonOfTypeFind_1.wrapCheckBuilderJsonOfTypeFind)(typeof path === "function" ? jvm_types_1.CoreDsl.jmesPath((0, session_1.underlyingSessionTo)(path)) : jvm_types_1.CoreDsl.jmesPath(path));
      exports.jmesPath = jmesPath2;
  ...
  __export(stdin_exports, {
    APIBanking: () => APIBanking_gatling_default,
    banking: () => banking_gatling_default,
    computerdatabase: () => computerdatabase_gatling_default
  });

  // src/APIBanking.gatling.ts
  var import_core = __toESM(require_target(), 1);
  var import_http = __toESM(require_target2(), 1);
  var APIBanking_gatling_default = (0, import_core.simulation)((setUp) => {
    const httpProtocol = import_http.http.baseUrl("https://api.example.com/banking").acceptHeader("application/json").connectionHeader("keep-alive").contentTypeHeader("application/json");
    const feeder = (0, import_core.csv)("transactions.csv").random();
    const myScenario = (0, import_core.scenario)("POST Transaction").exec(
      (0, import_http.http)("Get transaction by ID").get("/transactions/#{transactionsId}").check((0, import_http.status)().is(200)).check((0, import_core.jsonPath)("$.transaction_id").is("#{transactionsId}")).check((0, import_core.jsonPath)("$.from_account").is("${from_account}")).check((0, import_core.jsonPath)("$.to_account").is("${to_account}")).check((0, import_core.jsonPath)("$.amount").is("${amount}")).check((0, import_core.jsonPath)("$.currency").is("${currency}")),
      (0, import_core.pause)(5),
      (0, import_http.http)("Get transaction by ID").get("T /accounts/#{account_id}/balance").check((0, import_http.status)().is(200)).check((0, import_core.jsonPath)("$.account_id").is("#{account_id}")).check((0, import_core.jsonPath)("$.balance").is("${balance}")).check((0, import_core.jsonPath)("$.currency").is("${currency}")),
      (0, import_core.pause)(2),
      (0, import_core.feed)(feeder),
      (0, import_http.http)("Create transaction: #{from_account}, #{to_account}, #{amount}, #{currency}").post("/transactions").body(
        (0, import_core.StringBody)('{"from_account": "${from_account}", "to_account": "${to_account}", "amount": "${amount}", "currency": "${currency}"}')
      ).asJson().check((0, import_core.jmesPath)("transaction_id").is("#{transaction_id}")).check((0, import_core.jmesPath)("new_balance").is("#{new_balance}")).check((0, import_core.jmesPath)("success").is("#{success}"))
    );
    setUp(
      myScenario.injectOpen(
        (0, import_core.atOnceUsers)(5),
        (0, import_core.rampUsers)(10).during(10)
      ).protocols(httpProtocol)
    );
  });

  // src/banking.gatling.ts
  var import_core2 = __toESM(require_target(), 1);
  var import_http2 = __toESM(require_target2(), 1);
  var banking_gatling_default = (0, import_core2.simulation)((setUp) => {
    const feeder = (0, import_core2.csv)("transactions.csv").random();
    const search = (0, import_core2.exec)(
      (0, import_http2.http)("Home").get("/"),
      (0, import_core2.pause)(1)
    );
    const edit = (0, import_core2.tryMax)(2).on(
      (0, import_core2.feed)(feeder),
      (0, import_http2.http)("Form").get("/"),
      (0, import_core2.pause)(1),
      (0, import_http2.http)("Post").post("/").formParam("fromAccount", "#{from_account}").formParam("toAccount", "#{to_account}").formParam("amount", "#{amount}").formParam("currency", "#{currency}").check(
    ...
    const httpProtocol = import_http2.http.baseUrl("http://localhost:8000/banking.html").acceptHeader("text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8").acceptLanguageHeader("en-US,en;q=0.5").acceptEncodingHeader("gzip, deflate").userAgentHeader(
      "Mozilla/5.0 (Macintosh; Intel Mac OS X 10.15; rv:109.0) Gecko/20100101 Firefox/119.0"
    );
    const browse = (0, import_core2.repeat)(4, "i").on((0, import_http2.http)("Page #{i}").get("/"), (0, import_core2.pause)(1));
    const users = (0, import_core2.scenario)("Users").exec(browse, edit);
    setUp(
  ...
  var computerdatabase_gatling_default = (0, import_core3.simulation)((setUp) => {
    const feeder = (0, import_core3.csv)("search.csv").random();
    const search = (0, import_core3.exec)(
      (0, import_http3.http)("Home").get("/"),
      (0, import_core3.pause)(1),
      (0, import_core3.feed)(feeder),
      ...
      (0, import_core3.pause)(1),
      (0, import_http3.http)("Select").get("#{computerUrl}").check((0, import_http3.status)().is(200)),
      (0, import_core3.pause)(1)
    );
    const browse = (
      // Note how we force the counter name, so we can reuse it
      (0, import_core3.repeat)(4, "i").on((0, import_http3.http)("Page #{i}").get("/computers?p=#{i}"), (0, import_core3.pause)(1))
    );
    const edit = (
      // let's try at max 2 times
      (0, import_core3.tryMax)(2).on(
        (0, import_http3.http)("Form").get("/computers/new"),
        (0, import_core3.pause)(1),
        (0, import_http3.http)("Post").post("/computers").formParam("name", "Beautiful Computer").formParam("introduced", "2012-05-30").formParam("discontinued", "").formParam("company", "37").check(
          (0, import_http3.status)().is(
    ...
    const httpProtocol = import_http3.http.baseUrl("https://computer-database.gatling.io").acceptHeader("text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8").acceptLanguageHeader("en-US,en;q=0.5").acceptEncodingHeader("gzip, deflate").userAgentHeader(
      "Mozilla/5.0 (Macintosh; Intel Mac OS X 10.15; rv:109.0) Gecko/20100101 Firefox/119.0"
    );
    const users = (0, import_core3.scenario)("Users").exec(search, browse);
    const admins = (0, import_core3.scenario)("Admins").exec(search, browse, edit);
    setUp(
      users.injectOpen((0, import_core3.rampUsers)(10).during(10)),
      admins.injectOpen((0, import_core3.rampUsers)(2).during(10))
    ).protocols(httpProtocol);
  });
